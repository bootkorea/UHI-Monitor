...
import static com.example.heatislandmap.MainActivity.HEAT_ISLAND_TEMP1;
    ...
    protected static class TempThread extends Thread{
        public void run() {
            try {
                ...
                double avg_of_avgTemp = 0.0; //평균 기온의 평균
                LocalTime current_time = LocalTime.now();
                int current_hour = current_time.getHour();
                HEAT_ISLAND_AVG = 0.0;
                for (int i = 0; i < REGION_NAME.size(); i++) { // *** 요 반복 횟수가 많을수록 실행시간 증가.. *** ( 원래는 i < REGION_NAME.size() )
                    avg_temp = 0.0;
                    double sum = 0.0; // 각 시각에 구한 평균기온의 합
                    String autonomous = null; //자치구 이름
                    int time = (current_hour + 9) % 24; //원하는 시간대 설정 가능
                    ...
                    //if(i == 0) HEAT_ISLAND_AVG = avg_temp_arr[0];
                    //HEAT_ISLAND_AVG = Math.min(HEAT_ISLAND_AVG, avg_temp_arr[i]);
                ...
                //System.out.println(avg_of_avgTemp);
